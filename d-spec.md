...
- 기본 모델: `anthropic/claude-opus-4.5` (코호트 추론 품질 우선). 빠른 대안: `anthropic/claude-opus-4.5`.
- ⚠ **모델 변경 전 검증 필수.** InsForge OpenRouter 라우팅이 활성-but-호출-실패 패턴 있음. 검증된 동작 모델: `anthropic/claude-opus-4.5`, `anthropic/claude-sonnet-4.5`, `anthropic/claude-opus-4.5`, `deepseek/deepseek-v3.2`, `x-ai/grok-4.1-fast`. **막혀 있는 라인**: OpenAI gpt-5 / gpt-5.2 / gpt-5.4 / gpt-5.5 (enable=201, call=500 AI_UPSTREAM_UNAVAILABLE). 모델 핀 전에 `POST /api/ai/chat/completion` 한 번 찔러서 200 확인.
...
- **Fallback path (LLM mode):** events를 마크다운 리스트로 포맷 → InsForge AI Gateway (`insforge.ai.chat.completions.create({ model: 'anthropic/claude-opus-4.5', messages: [...] })`)에 prompt: "User events:\n{events}\nQuestion: {question}\nAnswer concisely citing event timestamps." → 응답 반환
