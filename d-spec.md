...
- API key 캡처
- **결정:** Nia 인덱싱 모드 — Sync daemon vs 직접 업로드 API 중 더 빠른 쪽. 5분 안에 결정 못 하면 **inline LLM fallback**으로 즉시 전환 (InsForge AI Gateway로 events를 컨텍스트 주입, 모델 `anthropic/claude-3.5-haiku`).
...
- 기본 모델: `anthropic/claude-3.5-haiku` (저렴+빠름). 대안: `openai/gpt-4o-mini`.
...
- **Fallback path (LLM mode):** events를 마크다운 리스트로 포맷 → InsForge AI Gateway (`insforge.ai.chat.completions.create({ model: 'anthropic/claude-3.5-haiku', messages: [...] })`)에 prompt: "User events:\n{events}\nQuestion: {question}\nAnswer concisely citing event timestamps." → 응답 반환
