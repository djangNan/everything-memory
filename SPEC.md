...
- **Fallback if Nia indexing latency is too high:** assemble the user's event log inline and pass it as context to a single InsForge AI Gateway call (`anthropic/claude-opus-4.5`) from the edge function. (Decided at build time depending on Nia daemon availability.)
...
| LLM (NL query synth) | **InsForge AI Gateway** (`anthropic/claude-opus-4.5`) | Per team decision: OpenAI family. One key (InsForge); credits billed via InsForge AI pool |
